...
class CountingApples(Scene):
    def construct(self):
        ...
        bunny = VGroup(bunny_body, bunny_head, left_ear, right_ear).scale(
            0.5).to_edge(LEFT).shift(DOWN*0.5)
        ...
        red_apples.next_to(bunny, RIGHT, buff=1)

        # Counting the first five apples
        self.play(LaggedStart(*[Create(apple)
        ...
        for i in range(8):
            ...
            count_text = Text(
                str(i + 1), font_size=40).next_to(all_apples, DOWN)
