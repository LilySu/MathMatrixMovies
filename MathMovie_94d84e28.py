...
class AdditionExample(VoiceoverScene):
    def construct(self):
        ...
        # Stars
        red_stars = VGroup(*[Star(color=RED, fill_opacity=1).shift(2*i*LEFT) for i in range(3)])
        green_stars = VGroup(*[Star(color=GREEN, fill_opacity=1).shift(2*i*RIGHT) for i in range(5)])

        self.play(LaggedStart(*[FadeIn(star, scale=0.6, rate_func=there_and_back) for star in red_stars], lag_ratio=0.2))
        self.play(LaggedStart(*[FadeIn(star, scale=0.6, rate_func=there_and_back) for star in green_stars], lag_ratio=0.2))
        ...
        self.play(
            LaggedStart(
                *[
                    ApplyMethod(star.move_to, target, rate_func=there_and_back)
        ...
        self.play(all_stars.animate.scale(1.2))
