...
class AddingSixAndNine(VoiceoverScene):
    def construct(self):
        ...
        # Basket 
        basket = VGroup(
            Rectangle(width=4, height=1.5, color=constants.BROWN, fill_opacity=1),  # Importing BROWN from manim.constants
            Line(start=[-1, 0.75, 0], end=[1, 2, 0], color=constants.BROWN, stroke_width=8),
            Line(start=[1, 0.75, 0], end=[-1, 2, 0], color=constants.BROWN, stroke_width=8)
        ).to_edge(DOWN)
        ...
        count = 0
        for fruit in fruits:
            self.play(fruit.animate.move_to(basket.get_center() + np.random.uniform(-0.5, 0.5, 2)))
            count += 1
