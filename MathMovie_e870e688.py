...
class FiniteFields(VoiceoverScene):
    def construct(self):
        ...
        self.play(Rotate(hour_hand, angle=(5/12)*TAU, about_point=clock.get_center), run_time=2) # Adjusted run time
        ...
        self.play(field_elements[3].animate.set_color(YELLOW), field_elements[4].animate.set_color(YELLOW), run_time=1) # Adjusted run time
        ...
        with self.voiceover(text="But since 7 is bigger than our limit, we subtract 5, leaving us with 2.") as tracker:
            self.play(Transform(temp_sum, Text("-5", color=RED).move_to(temp_sum.get_center()).scale(0.7)), run_time = 1) # Adjusted run time
            self.play(
            ...
            self.play(Indicate(field_elements[2]), run_time=1.5) # Adjusted run time
            self.play(field_elements[3].animate.set_color(WHITE), field_elements[4].animate.set_color(WHITE), run_time = 1) # Adjusted run time
