...
class AdditionExample(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text="Now, if we count all the apples together, how many do we have?") as tracker:
            self.play(
                # Placeholder animation to keep the voiceover synchronized
                AnimationGroup(
                    FadeToColor(all_apples, color=YELLOW, rate_func=there_and_back),
        ...
        with self.voiceover(text="That's right! We have ten apples in total. So, 3 + 7 equals 10!") as tracker:
            self.play(
                ...
                AnimationGroup(
                    FadeToColor(total_number, color=WHITE, rate_func=there_and_back),
                    run_time=tracker.duration
                )
            )
        self.wait(0.5)
        ...
        with self.voiceover(text="See? Addition is all about combining things together and finding the total number.") as tracker:
            self.play(
                # Placeholder animation
                AnimationGroup(
                    FadeToColor(equation, color=PINK, rate_func=there_and_back),
                    run_time=tracker.duration
                )
            )
        self.wait(1)
